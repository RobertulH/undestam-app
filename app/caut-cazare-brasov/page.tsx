...
function isBrasovLike(text: string) {
  const t = (text || "").toLowerCase();
  return (
    t.includes("brașov") ||
    t.includes("brasov") ||
    t.includes("poiana") ||
    t.includes("poiana brasov") ||
    t.includes("râșnov") ||
    t.includes("rasnov") ||
    t.includes("predeal") ||
    t.includes("azuga")
...
export default function CautCazareBrasov() {
  ...
  const filtered = useMemo(() => {
    ...
    return requests
      .filter((r) => isBrasovLike(`${r.title} ${r.location}`))
      .filter((r) => {
    ...
    <div style={{ minHeight: "100vh", padding: 18, background: "#0b1220", color: "#e8eefc", fontFamily: "system-ui" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <p style={{ color: "#a9b7d6", lineHeight: 1.7 }}>
          Cereri pentru Brașov, Poiana Brașov, Predeal, Râșnov, Azuga. Creează o cerere și primești recomandări reale.
        </p>

        <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap" }}>
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="caută: pârtie, mic dejun..." style={{ borderRadius: 12, padding: 10, border: "1px solid rgba(255,255,255,.1)", background: "rgba(255,255,255,.04)", color: "#e8eefc", minWidth: 240 }} />
        ...
        <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 14 }}>
          {filtered.length === 0 ? (
            <div style={{ gridColumn: "1 / -1", padding: 14, borderRadius: 14, border: "1px solid rgba(255,255,255,.1)", background: "rgba(255,255,255,.03)" }}>
              Nu am găsit cereri pentru Brașov (încă). Creează una din homepage.
            </div>
          ) : (
            filtered.map((r, idx) => (
